...
class TestNtvTabular(unittest.TestCase):
    ...
    def test_tab_field_pandas_ilist_Iindex(self):
        field = Ntv.obj(
            {":field": {"dates::datetime": ["1964-01-01", "1985-02-05", "2022-01-21"]}}
        )
        tab = Ntv.obj(
            {
                ":tab": {
        ...
        self.assertTrue(
            sr.equals(
                Ntv.obj(sr).to_obj(format="obj", dicobj={"field": "SeriesConnec"})
            )
        )
        ...
        self.assertTrue(
            df.equals(
                Ntv.obj(df).to_obj(format="obj", dicobj={"tab": "DataFrameConnec"})
            )
        )
...
class TestNTVpandas(unittest.TestCase):
    ...
    def test_series(self):
        ...
        srs = [  # without ntv_type, without dtype
            ...
            pd.Series(
                [{"a": 2, "e": 4}, {"a": 3, "e": 5}, {"a": 4, "e": 6}], name="::object"
            ),
            ...
            pd.Series(
                [datetime.date(2022, 1, 1), datetime.date(2022, 1, 2)], name="::date"
            ),
            pd.Series(
                [datetime.time(10, 21, 1), datetime.time(8, 1, 2)], name="::time"
            ),
            ...
            pd.Series(
                [
                    ...
                    Polygon(
                        [[1.0, 2.0], [1.0, 30.0], [30.0, 30.0], [30, 2]],
        ...
        for sr in srs:
            # print(Ntv.obj(sr))
            self.assertTrue(
                npd.as_def_type(sr).equals(Ntv.obj(sr).to_obj(format="obj"))
            )
            ...
            self.assertTrue(
                npd.as_def_type(sr).equals(npd.read_json(npd.to_json(sr, table=True)))
            )
    ...
    def test_dataframe(self):
        ...
        for df in [
            pd.DataFrame(
                {
                    "test::date": pd.Series(
                        [date(2021, 1, 5), date(2021, 1, 5), date(2021, 1, 5)]
                    ),
                    "entiers": pd.Series([1, 2, 3]),
                }
            ),
        ]:
...
class TestExports(unittest.TestCase):
    ...
    def test_dataframe(self):
        ...
        df_fruits_sort = df_fruits.sort_values(list(df_fruits.columns)).reset_index(
            drop=True
        )
