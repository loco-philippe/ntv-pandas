...
class Test_table_pandas(unittest.TestCase):
    ...
    def test_series(self):
        ...
        for ind, srs in enumerate([# without ntv_type, without dtype
            ...
            self.assertTrue(npd.as_def_type(srs).equals(npd.read_json(npd.to_json(srs, table=True))))
            self.assertEqual(npd.read_json(npd.to_json(srs, table=True)).name, srs.name)            
